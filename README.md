...
리치를 선언했을때 하는 행동들
1. 리치 한다.
2. 점수 -1000점 하고 리치 표시한다.
3. 리치 선언한 턴의 버림 패를 가로로 돌려놓는다.
4. 리치 후에는 뽑은 패를 바로 버리면 된다.
...
# 머리는?
- 커쯔에서 하나 부족한 패로도 볼 수 있다.

# 핸드에서 텐파이인지 계산하는 케이스 정리

1. 커쯔를 뺀다. (같은거 3개)
2. 슌쯔를 뺀다. (123,456)
3. 머리를 뺸다.

* 커쯔 판별법 -> 읽는다 -> 다음 -> 다음다음 까지 같으면 커쯔
* 슌쯔 판별법 -> 읽는다 -> 같으면 컨티뉴 -> 다음이 +1 이면 반복 -> 다음다음이 +1이면 슌쯔
...
# 🀇🀇🀈🀈🀉🀉🀊🀊🀞🀟🀡
2. 슌쯔를 뺀다.
* 1만 읽고 1만 current에 담아놔, 세는것 count +1
* 다음 1만을 읽었다, 똑같음. 컨티뉴
* 2만 읽고 1만과 비교, +1차이 이므로 bodyIndex에 기록, current에 2만을 담아둔다.
* 다음 2만을 읽고 비교, 똑같음. 컨티뉴
* 3만을 읽고 비교, +1 차이 이므로 count +1 , 슌쯔로 판별하고 루프 종료.
* bodyIndex의 내용을 계산하여 업데이트 한다.
* 내패의 끝까지 판별한다. -> 재귀로 해보자
    * 종료 조건
        * 내 핸드가 3개 미만으로 남았다 (헤드밖에 없음)
        * bodyIndex가 차지 않았다? 혹은 지금 current가 리스트의 마지막이다?
...
# 🀞🀟🀡
4. 몸통 갯수 & 머리 갯수에 따라 리치 가능인지 정리.
- 몸통이 4개 (남은 패가 2장)
    - 무조건 리치
    - 한장 + a가 머리가 되면 되므로 뭘 버려도 리치임
- 몸통이 3개 & 머리가 있다
    - 남은 타일 수는 3개
        - 머리가 1개 더있다 -> 리치
        - 몸통이 2개까지 있음 -> 리치
        - 3개가 각각 따로 논다 -> 리치 아님
- 몸통 3개 이하만 있음 -> 뭘해도 한개버려서는 리치가 안나옴.
